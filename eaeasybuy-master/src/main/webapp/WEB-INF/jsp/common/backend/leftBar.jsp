...
<div class="m_left">
    ...
    <div class="left_m">
        ...
        <ul>
            <li><a href="${ctx}/admin/order?action=index&userId=${sessionScope.loginUser.id}"
                    <c:if test="${menu==1}"> class="now" </c:if>>我的订单</a></li>
            <c:if test="${sessionScope.loginUser.type==1}">
                <li><a href="${ctx}/admin/order?action=queryAllOrder"
    ...
    <c:if test="${sessionScope.loginUser.type==1}">
        <div class="left_m">
            <div class="left_m_t t_bg2">投诉和建议</div>
            <ul>
                <li><a href="${ctx}/admin/guestbook"  <c:if test="${menu==10}"> class="now" </c:if>>投诉和建议</a></li>
            </ul>
        </div>
    </c:if>
</div>